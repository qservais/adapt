  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => {
        ...
        const pillBg = isDone ? `${modeColor}20` : isCurrent ? `${modeColor}30` : COLORS.bgElevated;
        const pillBorder = isDone ? `${modeColor}60` : isCurrent ? modeColor : COLORS.border;
        const numColor = isDone ? modeColor : isCurrent ? modeColor : COLORS.textMuted;

        return (
          <View style={[styles.ribbonPill, { backgroundColor: pillBg, borderColor: pillBorder }, isCurrent && styles.ribbonPillCurrent]}>
            {isDone ? (
              <Feather name="check" size={11} color={modeColor} />
            ) : (
              <Text style={[styles.ribbonNum, { fontFamily: FONTS.mono, color: numColor }, isUpcoming && { opacity: 0.5 }]}>
            ...
            <Text
              ...
              numberOfLines={1}
            >
              {item.exerciseName.length > 10 ? item.exerciseName.substring(0, 9) + "…" : item.exerciseName}
            </Text>
          </View>
        );
      }}
...
export default function FreeExerciseScreen() {
  ...
  const handleSetDone = () => {
    if (hasDurationTimer && !timerCompleted) {
      Alert.alert("Chrono obligatoire", "Lance et termine le chrono avant de valider la série.", [{ text: "OK" }]);
      return;
    }
  ...
  return (
    <View style={[styles.flex, { backgroundColor: COLORS.bg }]}>
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <View style={{ flex: 1 }}>
          <ProgressBar progress={exerciseIndex} total={totalExercises} color={cfg.color} height={4} />
        </View>
      ...
      <View style={styles.identityBlock}>
        ...
        <Text style={[styles.repsText, { fontFamily: FONTS.mono }]}>
          {hasDurationTimer ? `${exercise.durationSeconds}s` : `${exercise.reps} REPS`}
        </Text>
      </View>
      ...
      >
          ...
          <View style={styles.descriptionBox}>
            <View style={styles.descriptionHeader}>
              ...
              {exercise.demoUrl != null && exercise.demoUrl.length > 0 && (
                <TouchableOpacity onPress={() => setShowDemoModal(true)} style={[styles.demoBtn, { borderColor: `${cfg.color}50` }]}>
                  <Feather name="play-circle" size={13} color={cfg.color} />
                  <Text style={[styles.demoBtnText, { fontFamily: FONTS.bodyMedium, color: cfg.color }]}>Voir la démo</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity onPress={() => setShowDescription(v => !v)} style={styles.descriptionToggle}>
                <Text style={[styles.descriptionToggleText, { fontFamily: FONTS.body }]}>
                  {showDescription ? "Masquer" : "Voir"}
                </Text>
                <Feather name={showDescription ? "chevron-up" : "chevron-down"} size={13} color={COLORS.textMuted} />
              </TouchableOpacity>
            </View>
            {showDescription && (
        ...
        {currentLoad > 0 && (
          <View style={styles.loadSection}>
            <Stepper value={currentLoad} onChange={(v) => adjustLoad(exercise.id, v)} min={0} max={500} step={2.5} unit="kg" label="CHARGE" decimals={1} />
            {lastUsedLabel != null && (
              <View style={styles.lastUsedRow}>
                <Feather name="clock" size={11} color={COLORS.textMuted} />
                <Text style={[styles.lastUsedText, { fontFamily: FONTS.mono }]}>{lastUsedLabel}</Text>
              </View>
            )}
        ...
        {hasDurationTimer && !showRest && (
          <View style={styles.workTimerContainer}>
            <CircularTimer ref={workTimerRef} durationSeconds={exercise.durationSeconds!} onComplete={handleWorkTimerComplete} autoStart={false} label="TRAVAIL" />
            {!workTimerStarted && !timerCompleted && (
              <TouchableOpacity onPress={handleWorkTimerStart} style={[styles.startTimerBtn, { backgroundColor: cfg.color }]}>
                <Feather name="play" size={16} color={COLORS.bg} />
                <Text style={[styles.startTimerText, { fontFamily: FONTS.bodyBold }]}>Lancer le chrono</Text>
              </TouchableOpacity>
            )}
            {workTimerStarted && !timerCompleted && (
              <TouchableOpacity onPress={() => setTimerCompleted(true)} style={styles.skipBtn}>
                <Text style={[styles.skipText, { fontFamily: FONTS.bodyMedium }]}>Passer</Text>
              </TouchableOpacity>
            )}
            {timerCompleted && (
              <View style={[styles.timerDoneBadge, { backgroundColor: `${COLORS.green}20`, borderColor: `${COLORS.green}50` }]}>
                <Feather name="check-circle" size={14} color={COLORS.green} />
                <Text style={[styles.timerDoneText, { fontFamily: FONTS.monoBold, color: COLORS.green }]}>CHRONO TERMINÉ</Text>
              </View>
            )}
          </View>
        )}
        ...
        {showRest && (exercise.restSeconds ?? 0) > 0 ? (
          <View style={styles.restContainer}>
            <CircularTimer ref={timerRef} durationSeconds={exercise.restSeconds!} onComplete={handleRestTimerComplete} autoStart={false} />
            <View style={styles.encourageBox}>
              <Text style={[styles.encourageText, { fontFamily: FONTS.bodyMedium }]}>{encouragementMsg}</Text>
            </View>
            <TouchableOpacity onPress={handleSkipRest} style={styles.skipBtn}>
              <Text style={[styles.skipText, { fontFamily: FONTS.bodyMedium }]}>Passer le repos</Text>
            </TouchableOpacity>
          </View>
        ) : null}
        ...
        {nextExercise != null && (
          <View style={[styles.nextExRow, { borderColor: `${cfg.color}20` }]}>
            <View style={styles.nextExLeft}>
              <Text style={[styles.nextExLabel, { fontFamily: FONTS.mono, color: COLORS.textMuted }]}>SUIVANT</Text>
              <Text style={[styles.nextExName, { fontFamily: FONTS.bodyMedium }]} numberOfLines={1}>{nextExercise.exerciseName}</Text>
            </View>
            <Text style={[styles.nextExVolume, { fontFamily: FONTS.mono }]}>{nextExercise.sets}×{nextExercise.reps}</Text>
          </View>
        )}
      </ScrollView>
      ...
      {!showRest && (
        <View style={[styles.footer, { paddingBottom: insets.bottom + 24 }]}>
          <TouchableOpacity
            ...
            style={[styles.doneBtn, { backgroundColor: doneBtnDisabled ? COLORS.bgElevated : cfg.color }]}
          ...
          {doneBtnDisabled && (
            <Text style={[styles.timerHint, { fontFamily: FONTS.body }]}>Lance et termine le chrono pour valider</Text>
          )}
        </View>
      )}

      <Modal visible={showDemoModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowDemoModal(false)}>
        <View style={[styles.modalContainer, { paddingTop: insets.top }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { fontFamily: FONTS.mono }]} numberOfLines={1}>{exercise.exerciseName}</Text>
            <TouchableOpacity onPress={() => setShowDemoModal(false)} style={styles.modalCloseBtn}>
              <Feather name="x" size={20} color={COLORS.white} />
            </TouchableOpacity>
          </View>
          {exercise.demoUrl != null && (
            <WebView source={{ uri: exercise.demoUrl }} style={styles.webView} allowsInlineMediaPlayback mediaPlaybackRequiresUserAction={false} startInLoadingState />
          )}
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  flex: { flex: 1 },
  header: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingBottom: 10, gap: 10, backgroundColor: COLORS.bg },
  navBtn: { padding: 4 },
  navBtnBg: { width: 36, height: 36, borderRadius: 18, backgroundColor: COLORS.bgElevated, alignItems: "center", justifyContent: "center" },
  ...
  identityBlock: { alignItems: "center", paddingHorizontal: 24, paddingTop: 8, paddingBottom: 8, gap: 4, backgroundColor: COLORS.bg },
  ...
  content: { paddingHorizontal: 24, alignItems: "center", gap: 24, paddingTop: 16 },
  loadSection: { alignItems: "center", gap: 14, width: "100%" },
  lastUsedRow: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: -6 },
  lastUsedText: { fontSize: 11, color: COLORS.textMuted, letterSpacing: 0.3 },
  prInfo: { flexDirection: "row", alignItems: "center", gap: 6 },
  prText: { fontSize: 11, color: COLORS.textMuted, letterSpacing: 1 },
  workTimerContainer: { alignItems: "center", gap: 16, width: "100%" },
  startTimerBtn: { flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 12, paddingHorizontal: 24, paddingVertical: 14 },
  startTimerText: { fontSize: 15, color: COLORS.bg },
  timerDoneBadge: { flexDirection: "row", alignItems: "center", gap: 6, borderRadius: 12, borderWidth: 1, paddingHorizontal: 16, paddingVertical: 8 },
  timerDoneText: { fontSize: 12, letterSpacing: 1 },
  cueBox: { flexDirection: "row", alignItems: "flex-start", gap: 10, backgroundColor: COLORS.cyanDim, borderRadius: 12, borderWidth: 1, borderColor: `${COLORS.cyan}30`, padding: 14, width: "100%" },
  cueText: { flex: 1, fontSize: 14, color: COLORS.white, lineHeight: 20 },
  restContainer: { alignItems: "center", gap: 20, width: "100%" },
  encourageBox: { backgroundColor: COLORS.bgCard, borderRadius: 12, paddingHorizontal: 20, paddingVertical: 14, width: "100%" },
  encourageText: { fontSize: 15, color: COLORS.white, textAlign: "center", lineHeight: 22 },
  skipBtn: { paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: COLORS.border },
  skipText: { fontSize: 14, color: COLORS.textSecondary },
  nextExRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%", backgroundColor: COLORS.bgCard, borderRadius: 12, borderWidth: 1, paddingHorizontal: 16, paddingVertical: 12, gap: 12 },
  ...
  footer: { paddingHorizontal: 20, paddingTop: 12, backgroundColor: COLORS.bg, borderTopWidth: 1, borderTopColor: COLORS.border, gap: 8 },
  doneBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, borderRadius: 14, paddingVertical: 18 },
  doneBtnText: { fontSize: 16, letterSpacing: 1 },
  timerHint: { fontSize: 12, color: COLORS.textMuted, textAlign: "center", letterSpacing: 0.3 },
  backHomeBtn: { backgroundColor: COLORS.bgCard, borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12, borderWidth: 1, borderColor: COLORS.border },
  equipmentRow: { flexDirection: "row", alignItems: "flex-start", gap: 8, width: "100%" },
  equipmentTags: { flexDirection: "row", flexWrap: "wrap", gap: 6, flex: 1 },
  equipmentTag: { backgroundColor: COLORS.bgCard, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3, borderWidth: 1, borderColor: COLORS.border },
  equipmentTagText: { fontSize: 11, color: COLORS.textSecondary, letterSpacing: 0.3 },
  descriptionBox: { backgroundColor: COLORS.bgCard, borderRadius: 14, borderWidth: 1, borderColor: COLORS.border, padding: 14, gap: 10, width: "100%" },
  descriptionHeader: { flexDirection: "row", alignItems: "center", gap: 6 },
  descriptionLabel: { fontSize: 10, letterSpacing: 1.5, flex: 1 },
  descriptionToggle: { flexDirection: "row", alignItems: "center", gap: 3, paddingVertical: 2, paddingHorizontal: 4 },
  ...
  demoBtn: { flexDirection: "row", alignItems: "center", gap: 7, borderRadius: 8, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 8, alignSelf: "flex-start" },
  demoBtnText: { fontSize: 13 },
  modalContainer: { flex: 1, backgroundColor: COLORS.bg },
  modalHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: COLORS.border, gap: 12 },
