...
export function StepsSection() {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: { steps: number; goal?: number }) => {
      const today = new Date().toISOString().slice(0, 10);
      return customFetch("/api/stats/steps", {
        method: "POST",
        body: JSON.stringify({ date: today, steps: data.steps, goal: data.goal, source: "manual" }),
      });
    },
    onSuccess: () => {
  ...
  function openModal() {
    ...
    setStepsInput(today ? String(today.steps) : "");
